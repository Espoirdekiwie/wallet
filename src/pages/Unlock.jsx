 ...
 * Unlock.jsx
 * 
 * Dedicated Unlock page for stored encrypted wallets.
 * If no wallet exists in localStorage, automatically redirects to /import-wallet.
 */
...
import { FiLock, FiAlertTriangle, FiUnlock, FiTrash2, FiDownload } from 'react-icons/fi';
import { Card, Button, Input, showToast } from '../components';
...
function Unlock() {
  ...
  const { isUnlocked, unlockWallet, resetWallet } = useWallet();

  const [password, setPassword] = useState('');
  const [error, setError] = useState('');
  const [loading, setLoading] = useState(false);
  const [showResetModal, setShowResetModal] = useState(false);
  const [storedAddress] = useState(() => walletService.getStoredAddress());

  useEffect(() => {
    // 1. If already unlocked, redirect to dashboard
    if (isUnlocked) {
      navigate('/dashboard');
      return;
    }

    // 2. If walletData is missing, automatically redirect to /import-wallet
  ...
  const handleConfirmReset = () => {
    ...
    showToast.info('Wallet data cleared. Please import or create a new wallet.');
    navigate('/import-wallet');
  };
      ...
      >
        <Card className="p-4 p-md-5 text-center">
          ...
          <p className="text-muted small mb-4 font-mono">
            {storedAddress ? `Account: ${shortenAddress(storedAddress, 6)}` : 'Enter master password to access your assets'}
          </p>
          ...
          <div className="pt-3 border-top border-white border-opacity-10 d-flex flex-column gap-2">
            ...
            >
              <FiDownload className="me-1" /> Import a different wallet
            </Link>
            <button
              type="button"
              onClick={() => setShowResetModal(true)}
              className="btn btn-link text-danger text-decoration-none small font-mono p-0"
              style={{ fontSize: '0.85rem' }}
            >
